...
public class FlexVideoAction extends CommonAction {
	private static Logger logger=Logger.getLogger(FlexVideoAction.class);
	/**
	 * 
	 */
		...
		try{
			Map<String, Object> map = new HashMap<String, Object>();
				...
				if(urlList!=null && urlList.size()>0){
					bookUrl = urlList.get(0);
					if(urlList.size()>1){
						ypmpUrl = urlList.get(1);	
					}
				}
			...
			logger.error("FlexVideoAction.queryVideoInfo",e);
		}
	}
...
//			getQueryNotesCondition().setCusId(347);
			...
			logger.error("FlexVideoAction.getNotesListByPoint",e);
		}
		return pr;
	}
	...
	public Number queryVedioScores(int videoId){
		...
		if(videoId!=0){
			...
			logger.error("FlexVideoAction.queryVedioScores",e);
		}
		return voScores;
	}
