...
public class ChinaBankAction extends CommonAction {
	...
	private static final Logger logger = Logger.getLogger(ChinaBankAction.class);
	 ...
	 * @throws IOException
	 */
	public String goToChinaBank()  {
		try{
			logger.info("---------goToChinaBank--------");
			// 初始化定义参数
			getChinaBankInfo().setV_mid("22077726");
			getChinaBankInfo().setV_url(configurator.getProjectURL()+"/alipay/chinaBank!returnUrl.action");
			logger.info(configurator.getProjectURL()+"/alipay/chinaBank!returnUrl.action");
			//根据订单号及登陆用户id获取订单金额
			...
			logger.error("ChinaBankAction.goToChinaBank.",e);
			return ERROR;
		}
		...
		if(ct != null){
			...
			if(ct.getStatus() != Contract.CONTRACT_STATUS_PAIED){
				...
				try{
					...
					logger.error("ChinaBankAction.taskExecutor.CrmSendMessageTask",e);
				}
			}
	...
	private int getCusIdByParm() {
		if(getChinaBankInfo().getRemark1() != null && 
			...
			try {
				...
				logger.error("ChinaBankAction.getCusIdByParm.",e);
			}
		}
		return 0;
	}
	...
	private void sendBuySuccMsgToMobile(Customer cus) {
		if(cus.getMobile() != null && !"".equals(cus.getMobile().trim())){
			try {
				...
				logger.error("ChinaBankAction.sendBuySuccMsgToMobile.",e);
			}
		}
	}
...
//		List<CashRecord> crList = cashRecordService.getCashRecordByList(qcrc);//流水记录
	    ...
	    //---Yangning 2011/12/21  修改线上更新时同时更新支付类型--- begin // 
	    upcashRecord.setShopPayType(Contract.CONTRACT_PAY_TYPE_CB);
        this.getCashRecordService().upateCashRecordForOnlinePay(upcashRecord);
        //---Yangning 2011/12/21  修改线上更新时同时更新支付类型--- end // 
//		this.getCashRecordService().updateCashReocrdStatus(crList);
	...
	private void processTempContent() {
		try {
			...
			logger.error("ChinaBankAction.processTempContent.",e);
		}
	}
