...
import com.shangde.edu.res.domain.NotesList;
...
//		getQueryNotesCondition().setCusId(347);
	...
	public void saveOrUpdateNotes() throws Exception{
		Integer id = 0;
		try {
			if(getVideonotes()!= null && getLoginUserId()!=0){
				if(getVideonotes().getNoteId() == 0){
					getVideonotes().setUpdateTime(new Date());
					getVideonotes().setCusId(getLoginUserId());
					if(getVideonotes().getNoteInfo() == null || getVideonotes().getNoteInfo().trim().length() == 0){
						getVideonotes().setNoteInfo("");
					}
...
//				getQueryNotesCondition().setCusId(347);
	 ...
	 * @return Note ID
	 */
	public void getNotesByPointId(){
		try {
			getQueryNotesCondition().setPointId(getVideopointId());
			getQueryNotesCondition().setCusId(getLoginUserId());
			noteList = notesService.getNotesList(getQueryNotesCondition());
			NotesList nlist = new NotesList();
			nlist.setNoteList(noteList);
			JSONObject json = JSONObject.fromObject(nlist);
			post_outObj(json.toString());
			} catch (Exception e) {
				e.printStackTrace();
			} 
	}
...
//2010-08-27 Del By ZhangDong End
	 ...
	 * @thorows Exception
	 */
	public void deleteAllNotesByPointId() throws Exception{
		String jo ;
		try{
			notesService.deleteNotesByPointId(getVideopointId());
		}catch(Exception e){
			logger.error("删除当前视频的所有笔记出错");
			 jo = "false";
		}
		 jo = "true";
		post_outObj(jo.toString());
	...
	public String showNotesList() {
		try {
			...
			logger.error("NotesWebAction.showNotesList", e);
			return ERROR;
		}
		return "list";
	}
...
//			notesService.getNotesListByCus(getLoginUserId());
			...
			logger.error("NotesWebAction.showMyNotes", e);
			return ERROR;
		}
		return "showMyNotes";
	}
	...
	public String initUpdateNotes() {
		try {
			...
			logger.error("NotesWebAction.initUpdateNotes", e);
			return ERROR;
		}
		return "updatePage";
	}
