...
public class RoleImpl extends BaseService implements IRole {
	...
	public void addRoleRef(String userId,String roleId,String gradeIds){
		if(userId == null  ||  "".equals(userId.trim())  ||
		   ...
		   gradeIds == null  ||  "".equals(gradeIds.trim())) {
			return;
		}
		if(!StringUtils.isNullOrEmpty(gradeIds)){
			String str2[] = gradeIds.split(",");
			for(int i = 0 ; i < str2.length ; i++){
				String ids[] = str2[i].split(":");
				String subjectId = ids[0];
				String gradeId = ids[1];
				UserGradeSubjectRole ugsr = new UserGradeSubjectRole();
				ugsr.setRoleId(new Integer(roleId));
				ugsr.setUserId(new Integer(userId));
				ugsr.setSubjectId(new Integer(subjectId));
				ugsr.setGradeId(new Integer(gradeId));
