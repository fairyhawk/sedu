...
public class BackLoginAction extends CommonAction {
	...
	public String login() {
		...
		try{
			...
			User tempUser = userService.getUserByLoginname(user.getLoginName());
			//用户名长度
			...
			if (tempUser != null
					&& user != null
					&& tempUser.getLoginPwd()
					.equals(MD5.getMD5(user.getLoginPwd()))) {
				...
				tabUserFunctionList = processTabUserFunctionList(new ArrayList<Function>(curUserFunctionList));
