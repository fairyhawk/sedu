...
/**
 * SellCou�������ʵ����? User: guoqiang.liu Date: 2011-03-30
 */
...
public class SellCouImpl extends BaseService implements ISellCou {
	...
	public boolean GetSellWayByCode(String subjectId, String sellId) {
		...
		int val = simpleDao
				.getEntity("FinanceCoupon_NS.getSellWayForCode", map);
		if (val == 0)
			return false;
		else
			return true;
	...
	//只取状态因为如果已支付肯定状态早已改变
		int val = simpleDao
				.getEntity("FinanceCoupon_NS.getCouponState", Id);
		return val;
	}
	 ...
	 */
 	...
 	public Coupontype getCouponTypeByCouponId(int couponId){
 		return simpleDao.getEntity("CouponType_NS.getCouponTypeByCouponId", couponId);
 	}
     //谢添加结束
}
