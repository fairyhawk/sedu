...
function addbao(id, name, totalPrice, tupian, allclass, baselocation, isJump) {
	...
	gwybuy_bak=0;
	if (gwybuy>0){
		buyURL_bak="http://highso.org/static/web/column/273/index_1.shtml?gwybuy="+gwybuy;
		gwybuy_bak=gwybuy;
	}
	gwybuy=0;
	if (packageInfo != null) {
...
String.prototype.rtrim = function() {
	...
	function BuyNow(pid){ 
		...
		if(!(isLogin(baselocation + "/"))) {
		 	...
		 	BuyAjax(id);
		 	if (gwybuy>0){
				p.location="http://highso.org/static/web/column/273/index_1.shtml?gwybuy="+gwybuy;
				gwybuy=0;
			}else{
				p.location=buyURL;
			}
...
//								pageTracker._trackPageview('/buy/step_enter.html');
//							}
							return;
						}
