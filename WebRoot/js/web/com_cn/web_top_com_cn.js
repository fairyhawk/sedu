...
//		}
		...
		$("#regForm").validate({
	        ...
	        success: function(label) {
	        	...
	        	if(loginMessageFlag == "new") {
	        		label.html("<img src='" + importURL + "/images/web/com_cn/index/duihao_new.jpg'/>");
	        		return;
 	...
 	function toforpwd(){
 	 	window.open("http://haixue.com/static/web/column/112/index_1.shtml");
 	}
 	...
 	function showLoginInfo() {
 		if(isLogin()) {
			...
			if(loginMessageFlag == "new") {
				...
				//return;
			}else if(loginMessageFlag == "new3"){//首页的显示
				//$("#n_userinfo").html("<p>[<a href='javascript:showRegisterDiv()'>免费注册</a>]  [<a href='javascript:showLoginDiv()'>登录</a>]</p>");
 ...
 function showLoginedMessage(username) {
 		...
 		$.ajax({
			...
			success : function(result) {
				if(result.returnMessage == "success") {
					...
					if(url.indexOf("ss.haixue.com")!=-1){
						SetCookieDomain("sedu.cookie.ukey","","haixue.com");
	...
	function checkBack() {
		if(getParameter("back") == "true") {
			...
			//去掉端口号
			if(backURL.indexOf("haixue.com") != -1 && backURL.indexOf("http://") != -1) {
...
/*首页HighSo资讯*/
 function CpaKCtag(h){
	 for(i=0;i<6;i++){
		 document.getElementById("cpa_tag_b0"+i).style.display="none";
		 document.getElementById("cpa_tag_b0"+h).style.display="block";
		 document.getElementById("cpa_tag_t0"+i).style.background="#e3e3e3";
		 document.getElementById("cpa_tag_t0"+i).style.height="29px";
		 document.getElementById("cpa_tag_t0"+i).style.marginTop="2px";
		 document.getElementById("cpa_tag_t0"+h).style.background="#cbecb3";
		 document.getElementById("cpa_tag_t0"+h).style.height="31px";
		 document.getElementById("cpa_tag_t0"+h).style.marginTop="0px";
	 	}
	 }
	...
	function showRegisterPage() {
 	   ...
 	   window.location.href="http://haixue.com/register";
 	   if(typeof(_gaq)!="undefinde"&&_gaq!=null){
 	...
 	function showLoginPage() {
 		...
 		window.location.href="http://haixue.com/login";
 		if(typeof(_gaq)!="undefinde"&&_gaq!=null){
...
WebFromAgentAnalysis.prototype.analyse = function() {
	...
	var subjectId = this.webPaths[webPath][0];
