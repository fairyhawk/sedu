	...
	$.extend($.validator, {
		...
		prototype : {
			...
			init : function() {
				...
				function delegate(event) {
					...
					validator.settings["on" + event.type]
							&& validator.settings["on" + event.type].call(
									validator, this[0]);
				}
				$(this.currentForm).delegate("focusin focusout keyup",
						":text, :password, :file, select, textarea", delegate)
			...
			previousValue : function(element) {
				return $.data(element, "previousValue")
						|| $.data(element, "previousValue", previous = {
									old : null,
									valid : true,
		...
		methods : {
					...
					case 'select' :
						...
						return options.length > 0
								&& (element.type == "select-multiple" || ($.browser.msie
										&& !(options[0].attributes['value'].specified)
										? options[0].text
			...
			remote : function(value, element, param) {
				...
				this.settings.messages[element.name].remote = typeof previous.message == "function"
			...
			minlength : function(value, element, param) {
				return this.optional(element)
						|| this.getLength($.trim(value.replace(/[^\x00-\xff]/g,
										"**")), element) >= param;
			},
			...
			rangelength : function(value, element, param) {
				var length = this.getLength($.trim(value.replace(
								/[^\x00-\xff]/g, "**")), element);
				return this.optional(element)
			...
			email : function(value, element) {
				...
				return this.optional(element)
						|| /^([a-zA-Z0-9_.]+)@([a-zA-Z_1-9]+)[.]([a-zA-Z_1-9]+)$/.test(value);
//				return this.optional(element)
