...
//			pageTracker = _gat._getTracker('UA-22194725-8');
//		} else {
//			window.setTimeout(initPageTracker, 1000);
//		}
// 	}
...
//		}
 	...
 	function checkRegisterPram() {
 		if(isHomePage) {
 			$("<tr>" + 
	            ...
	            "	<option value='30'>远程学历</option>" + 
	            "	<option value='31'>银行从业考试</option>" +
	            "	<option value='32'>网络营销师</option>" +
	            "	<option value='33'>职称计算机</option>" +
	            "	<option value='34'>咨询工程师</option>" +
	            "	<option value='35'>安全工程师</option>" +
	            "	<option value='36'>招标师</option>" +
	            "	<option value='37'>护师考试</option>" +
	            "	<option value='38'>护士考试</option>" +
				"	<option value='39'>英语四六级</option>" +
				"	<option value='40'>中医执业医师</option>" +
				"	<option value='41'>英语雅思</option>" +
				"	<option value='42'>口腔执业医师</option>" +
				"	<option value='43'>公共卫生执业医师</option>" +
				"	<option value='44'>执业药师</option>" +
				"	<option value='45'>物业管理师</option>" +
				"	<option value='47'>保险代理人</option>" +
				"	<option value='48'>地方二级建造师</option>" +
	            "</select></td>" + 
	            "  </tr>" + 
	            "  <tr>" + 
	            "    <td>&nbsp;</td>" + 
 ...
 function showLoginedMessage(username) {
				...
				success : function(result) {
					if(result.returnMessage == "success") {
						...
						if(typeof(_gaq)!="undefinde"&&_gaq!=null){
							...
							_gaq.push(['_setCustomVar', 2, 'UserType', 'Registed', 1]);
							_gaq.push(['_setCustomVar', 3, 'SessionType', 'regist', 2]);
  							
							var google_conversion_id = 969121507;
							var google_conversion_language = "en";
							var google_conversion_format = "3";
							var google_conversion_color = "ffffff";
							var google_conversion_label = "duVhCIWy7AMQ472OzgM";
							var google_conversion_value = 0;
							
							(function() {
							var gaTemp = document.createElement('script'); 
							gaTemp.type = 'text/javascript'; 
							gaTemp.async = true;
							gaTemp.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.googleadservices.com/pagead/conversion.js';
							var s = document.getElementsByTagName('script')[0]; 
							s.parentNode.insertBefore(gaTemp, s);
							})(); 
							
						}
						var subjectIndexFlag = getSubjectIndexFlag(getCookie("subjectId"));
						
						//临时改变注册成功后的显示页面liuqg20120411-start
						/**
						 司法5、注会7、注税24、法律顾问22、成考23、远程学历30
						*/
						var subjectIndexFlagtmp =getCookie("subjectId")
						if(subjectIndexFlagtmp==5 || subjectIndexFlagtmp==7 ||subjectIndexFlagtmp==22 ||subjectIndexFlagtmp==23 ||subjectIndexFlagtmp==24 ||subjectIndexFlagtmp==30){
							doafterreg();
							window.location.href = baselocation+"/cus/cuslimit!toUserCenter.action";
							return;
						}
						//临时改变注册成功后的显示页面liuqg20120411-end
					...
					}  else if(result.returnMessage == "regMobileNone") {
						showErrorWin("手机号码不可为空。");
 	...
 	function getRegParams() {
 		var params = $("#regForm input,#regForm select").serialize();
		...
		var fromType = getCookie('_fromType');
		var wfaa = new WebFromAgentAnalysis();
		var path = wfaa.getWebPath();
		if (wfaa.webPaths[path] != null) {
			fromType = 1; // 项目页注册
		}
		if(fromType != null || fromType != '') {
			params+="&customer.fromType=" + fromType;
		}
 		return params;
 	}
	...
	function checkLoginTo(url) {
		if(typeof(_gaq)!="undefinde"&&_gaq!=null){
		_gaq.push(['_trackEvent', ' Entry', 'Study',' Index_Header']);  //谷歌统计 添加事件追踪统
		}
		forwordURL = url;
		if(hasLogin()) {
	...
	function checkLoginTo1(url) {
		if(typeof(_gaq)!="undefinde"&&_gaq!=null){
		_gaq.push(['_trackEvent', ' Entry', 'Order',' Index_Header']); //谷歌统计 添加事件追踪统
		}
		forwordURL = url;
		if(hasLogin()) {
			window.location.href = url;
	...
	function log_suc_examPractice(){
		//window.open(baselocation + "/exam/qstManager!getExamPaperAllList.action?subjectIdweb="+getCookie("subjectId")+"&queryExampaperCondition.currentPage=1");
		window.open(baselocation + "/exam/qstManager!getExampaperAnalysisDTO.action");
	}
	//讨论小组
	...
	function disBuyAndCouser(subectidd){
		...
		//购买课程显示	
		//getSellWayId(regsubid,"9");//去掉注册成功提示的购买信息
		...
		if (regsubid==1){ //人力资源管理师
			...
			$("#successListen_name1").html("建设工程施工管理");
			$("#successListen_name2").html("机电工程管理与实务");
	...
	function tolessionforreg(courseId){
   	 if(courseId){
   	 $.ajax({
				url : baselocation + "/cou/courselimit!getVUByCourseId.action",
				...
				success : function(result) {
					if(result.returnMessage == "success") {
						...
						window.open ('/cou/courselimit!toListenMyCourse.action?course.courseId=' + courseId+"&course.subjectId="+getCookie("subjectId")+"&veri=0&videoURL="+BASE64.encode(entity.videoURL)+"&kpointId="+entity.kpointId+"&sellid="+entity.SELL_ID+"&title="+entity.TITLE+"&videoId="+entity.VID, "_blank", "resizable");
					}else {
						window.location.href = baselocation + "/cus/cuslimit!toUserCenter.action";
					}
				},
	...
	function showRegisterPage() {
		if(getCookie('_fromType') != -1) {
			DeleteCookie('_fromType');
		}
		var wfaa = new WebFromAgentAnalysis();
		var path = wfaa.getWebPath();
		if(path == '') {
			SetCookie('_fromType', 4); // 首页注册
		} else if (wfaa.webPaths[path] != null){
			SetCookie('_fromType', 1); // 项目页注册
		}
 	   ...
 	   window.location.href="http://highso.cn/register";
 	...
 	function showLoginPage() {
 	    ...
 	    if(typeof(_gaq)!="undefinde"&&_gaq!=null){
			_gaq.push(['_trackEvent', ' Entry', 'Login',' Index_Header']); //谷歌统计 添加事件追踪统
               ...
               window.location.href="http://highso.cn/login";
