				...
				$.ajax({
					...
					success: function(result) {
						if(result==''){
							...
							if(contractInfo[0]!="" && webFrom!="" && (webFrom=='51bi' || webFrom=='weiyi')){
								$.ajax({
									url: baselocation +"/finance/sendCpsData!sendCpsData.action",
									type:"post",
									data:{ "webFrom" : webFrom,"contractId":contractInfo[0]}
								});
							}
						}
					},
		...
		if(checkAddr > 0){
			...
			$.ajax({
				...
				success : function(result) {
					...
					var webFrom = getCookie("webFrom"); 
					if(result.entity[0]!="" && (webFrom=='51bi' || webFrom=='weiyi')){
						$.ajax({
									url: baselocation +"/finance/sendCpsData!sendCpsData.action",
									data:{ "webFrom" : webFrom,"contractId":result.entity[0]}
								});
					}
				},
				error : function(error) {
