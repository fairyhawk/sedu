				...
				$.ajax({
					...
					success: function(result) {
						if(result==''){
							...
							flag = true;
							//下单成功,发送艾德思奇数据 网银支付
							getOrderInfo(contractInfo[0],contractInfo[1],2);
						}
					},
					error : function() {
						alert("error");
		...
		$.ajax({
			...
			success : function(result) {
				if(result.returnMessage == "success") {
					...
					//下单成功,发送艾德思奇数据 货到付款
					//getOrderInfo(result.entity[0],result.entity[1],1);
	...
	function createSendContract_gwy(){
		$.ajax({
			...
			success : function(result) {
				if(result.returnMessage == "success") {
					updateAddrFirstsf(result.entity);
					window.location.href="http://highso.net.cn/static/web/column/131/index_1.shtml";
				}
			},
			error : function(error) {
				alert('error');
