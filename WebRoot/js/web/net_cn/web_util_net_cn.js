...
function addbao(id, name, totalPrice, tupian, allclass, baselocation, isJump) {
	...
	gwybuy_bak=0;
	if (gwybuy>0){
		buyURL_bak="http://highso.net.cn/static/web/column/275/index_1.shtml?gwybuy="+gwybuy;
		gwybuy_bak=gwybuy;
	}
	gwybuy=0;
	if (packageInfo != null) {
...
String.prototype.rtrim = function() {
	...
	function BuyNow(pid){ 
		...
		if(!(isLogin(baselocation + "/"))) {
		 	...
		 	BuyAjax(id);
		 	if (gwybuy>0){
				p.location="http://highso.net.cn/static/web/column/275/index_1.shtml?gwybuy="+gwybuy;
				gwybuy=0;
			}else{
				p.location=buyURL;
			}
...
//给网盟发送订单数据
 	function docps(contractId){
 		var webFrom=getCookie("webFrom");
 		//亿起发，用SRC判断
 		var src=getCookie("SRC");
 		if (src!=null && src!="" ){
 			webFrom="yiqifa";
 			sendcpsdata(webFrom,contractId);
 			retrun;
 		}
 		//中视网盟,用webFrom判断
 		if(webFrom!=null && webFrom!="" && webFrom=="zswm"){
 			sendcpsdata(webFrom,contractId);
 			retrun;
 		}
 	...
 	function sendcpsdata(webfrom,contractId){
 		$.ajax({
			url : baselocation + "/finance/webcpscontract!sendCpsData.action",
			data : {"webFrom":webfrom,
